    ...
    private fun getCryptoDetailsForCurrency(cryptoForm: CryptoDetailsForm): CryptoDetailsForm {
        ...
        if (jsonObj.data.isNotEmpty()) {
            for (data in jsonObj.data) {
                ...
                val high24HourPrice = StringBuilder("High: ")
                high24HourPrice.append(DECIMAL_FORMAT.format(data.raw.gdp.high24Hour.toDoubleOrNull()))
                val low24HourPrice = StringBuilder("Low: ")
                low24HourPrice.append(DECIMAL_FORMAT.format(data.raw.gdp.low24Hour.toDoubleOrNull()))
                ...
                cryptoDetails.high24HourPrice = high24HourPrice.toString()
                cryptoDetails.low24HourPrice  = low24HourPrice.toString()
